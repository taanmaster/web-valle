...
class Crud extends Component
{
    ...
    {
        $photo = IdentityProgramImage::find($photoId);
        if (!$photo) {
            return;
        }
    ...
    public function uploadPhotos(): void
    {
        ...
        foreach ($this->newPhotos as $photo) {
            IdentityProgramImage::create([
                'identity_program_id' => $this->entry->id,
    ...
    public function save()
    {
        ...
        if (IdentityProgram::where('slug', $slug)->exists()) {
            $slug .= '-' . Str::lower(Str::random(4));
        }

        $newEntry = IdentityProgram::create([
            'title'        => $this->title,
            'slug'         => $slug,
            'description'  => $this->description,
            'content_1'    => $this->content_1,
        ...
        foreach ($this->newPhotos as $photo) {
            IdentityProgramImage::create([
                'identity_program_id' => $newEntry->id,
                'image_path'          => $this->handleUpload($photo),
            ]);
        }
    ...
    protected function handleUpload($document): string
    {
        ...
        $filename  = 'identity_program_' . time() . '_' . Str::random(10) . '.' . $extension;
        $filepath  = 'identity-program/' . $filename;
    ...
    public function render()
    {
        ...
        return view('identity-program.utilities.crud', compact('existingPhotos'));
    }
}
