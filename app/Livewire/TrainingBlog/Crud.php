    ...
    public function save()
    {
        ...
        if ($this->entry !== null) {
            if ($this->hero_img instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile) {
                ...
                })->save(public_path('images/training-blog/' . $imageCoverName));

                if ($this->entry->hero_img && $this->entry->hero_img !== 'empty-image.jpg') {
                    $old = public_path('images/training-blog/' . $this->entry->hero_img);
        ...
        if (!empty($this->photos)) {
            foreach ($this->photos as $photo) {
                ...
                $photo->move(public_path('images/training-blog/'), $filename);

                TrainingBlogImage::create([
                    'training_blog_id' => $newEntry->id,
                    'image_path'       => 'images/training-blog/' . $filename,
                ]);
            }
        }

        return redirect()->route('training_blog.admin.edit', $newEntry->id);
    }

    public function render()
    {
        return view('training-blog.utilities.crud');
    }
}
