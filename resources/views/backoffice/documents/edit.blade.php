...
@slot('title') Editar Oficio @endslot
@endcomponent
        ...
        <div class="col-lg-10 mx-auto">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fas fa-edit me-2"></i> Editar Oficio</h5>
                    <span class="badge bg-light text-dark">{{ $document->folio }}</span>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('backoffice.documents.update', $document->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <select name="dependency_id" id="dependency_id" class="form-select @error('dependency_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($dependencies as $dependency)
                                        <option value="{{ $dependency->id }}" 
                                                ...
                                                {{ old('dependency_id', $document->dependency_id) == $dependency->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" 
                                       ...
                                       readonly>
                                <small class="text-muted">Director de tu dependencia: {{ $userDependency->name ?? 'N/A' }}</small>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <select name="priority" class="form-select @error('priority') is-invalid @enderror" required>
                                    <option value="baja" {{ old('priority', $document->priority) == 'baja' ? 'selected' : '' }}>Baja</option>
                                    <option value="alta" {{ old('priority', $document->priority) == 'alta' ? 'selected' : '' }}>Alta</option>
                                    <option value="urgente" {{ old('priority', $document->priority) == 'urgente' ? 'selected' : '' }}>Urgente</option>
                                </select>
                                @error('priority')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                    <option value="solicitud" {{ old('type', $document->type) == 'solicitud' ? 'selected' : '' }}>Solicitud</option>
                                    <option value="respuesta" {{ old('type', $document->type) == 'respuesta' ? 'selected' : '' }}>Respuesta</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12">
                                ...
                                <textarea name="body" 
                                          ...
                                          required>{{ old('body', $document->body) }}</textarea>
                                @error('body')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            <a href="{{ route('backoffice.documents.show', $document->id) }}" class="btn btn-secondary">
