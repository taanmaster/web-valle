                    ...
                    <div class="row w-100">
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <ul class="nav nav-tabs">
                                <li class="nav-item">
                                    <a class="nav-link {{ !request('date') ? 'active' : '' }}"
                                        href="{{ route('obligation.detail', [$dependency, $obligation->slug]) }}">Ver
                                        Todos</a>
                                </li>
                                @foreach ($dates as $date)
                                    <li class="nav-item">
                                        <a class="nav-link {{ request('date') == $date ? 'active' : '' }}"
                                            href="{{ route('document.filter', [$dependency, $obligation->slug, $date]) }}">{{ $date }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <livewire:front.transparency-obligations.table :dependency="$obligation->dependency->id" :type="$obligation->type" :selectedObligation="$obligation->id"
                            :mode="1" />
                                        ...
                                        @if ($document->s3_asset_url != null)
                                            ...
                                            @else
                                                <a href="{{ asset('files/documents/' . $document->filename) }}"
                                                    target="_blank" class="card gazette-card d-block">
                                        @endif
