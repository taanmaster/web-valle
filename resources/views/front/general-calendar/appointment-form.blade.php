<div>
    @push('styles')
        <style>
            .gcf-hero {
                border-radius: 16px;
                background: linear-gradient(135deg, #551312 0%, #7a2422 100%);
                min-height: 300px;
                display: flex;
                ...
                <div class="col-lg-5">
                    ...
                    @if ($this->selectedProcedure)
                        <div class="mb-3">
                            ...
                            @if ($diasDisponibles)
                                <small class="text-muted d-block mt-1">Días con citas disponibles:
                                    {{ $diasDisponibles }}.</small>
                            @else
                                <small class="text-muted d-block mt-1">Este trámite aún no tiene horarios de citas
                                    disponibles.</small>
                            @endif
                            @error('fecha')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                        ...
                        @if ($fecha)
                            <div class="mb-3">
                                ...
                                @if (empty($this->availableSlots))
                                    <small class="text-muted">No hay horarios disponibles para esta fecha; intenta con
                                        otra.</small>
                                @endif
                                @error('horario')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
