        ...
        @php
            ...
            $tramites_config = [
                'uso-de-suelo' => [
                    ...
                    'description' =>
                        'Solicitud para determinar el uso de suelo permitido en un predio específico según la zonificación municipal.',
                    'icon' => 'map-outline',
                    'color' => 'info',
                    'legal_base' =>
                        'Artículos 256, 257, 258, 259, 261, 262 y 264 del Código Territorial para el Estado y los Municipios de Guanajuato',
                    'costs' => [
                    ...
                    // Este trámite tiene dos variantes que comparten los mismos costos
                    // pero difieren en los pasos/requisitos.
                    'variants' => [
                        [
                            'title' => 'PUS de Bajo Impacto',
                            'steps' => [
                        ...
                        [
                            'title' => 'PUS de Mediano y Alto Impacto',
                            'steps' => [
                                'Formato único de solicitud. (FORMATO COMPLETO)',
                                'Identificación oficial.',
                                'Carta Poder, si actúa a nombre y representación del propietario.',
                ...
                'constancia-de-factibilidad' => [
                    ...
                    'description' =>
                        'Documento que certifica la viabilidad técnica y legal para desarrollar un proyecto específico en el predio.',
                    'icon' => 'checkmark-circle-outline',
                    'color' => 'warning',
                    'legal_base' =>
                        'Artículos 253, 254 y 255 del Código Territorial para el Estado y los Municipios de Guanajuato',
                    'costs' => [['description' => 'a) Constancias expedidas por la dependencia', 'price' => '$87.05']],
                    'steps' => [
                ...
                'permiso-de-anuncios' => [
                    ...
                    'description' =>
                        'Autorización para la instalación y operación de elementos publicitarios y toldos en el municipio.',
                ...
                'certificacion-numero-oficial' => [
                    ...
                    'description' =>
                        'Asignación oficial del número que corresponde a un predio según su ubicación y normativa municipal.',
                    'icon' => 'home-outline',
                    'color' => 'dark',
                    'legal_base' =>
                        'Artículos 68 y 72 del Reglamento de Construcción y Fisionomía para el Municipio de Valle de Santiago',
                    'costs' => [
                ...
                'permiso-de-division' => [
                    ...
                    'description' =>
                        'Autorización para dividir un predio en fracciones menores conforme a la normativa territorial vigente.',
                    'icon' => 'resize-outline',
                    'color' => 'success',
                    'legal_base' =>
                        'Artículos 395, 397 y 398 del Código Territorial para el Estado y sus Municipios de Guanajuato',
                    'costs' => [['description' => 'Costo', 'price' => '$387.87']],
                    'steps' => [
                ...
                'uso-de-via-publica' => [
                    ...
                    'description' =>
                        'Permiso temporal o permanente para ocupar espacios de vía pública con fines específicos.',
                    'icon' => 'car-outline',
                    'color' => 'info',
                    'legal_base' => 'Artículos 53, 54, 55 y 56 del Reglamento de Construcción de Valle de Santiago',
                    'costs' => [['description' => 'Costo', 'price' => '$4.69']],
                    'steps' => [
                        'Formato de solicitud para Licencia de Uso Suelo',
                        'Dicho formato deberá estar firmado por el propietario del predio, representante legal o el arrendador, según sea el caso.',
                        'Copia del último pago del predial.',
                ...
                'permiso-de-construccion' => [
                    ...
                    'description' =>
                        'Autorización oficial para realizar obras de construcción, modificación o ampliación de inmuebles.',
                    'icon' => 'construct-outline',
                    'color' => 'danger',
                    'legal_base' =>
                        'Artículo 24 fracciones I inc. a2a y a2b del Reglamento de Construcción de Valle de Santiago',
                    'costs' => [
                ...
                'permiso-construccion-panteones' => [
                    ...
                    'description' =>
                        'Autorización especial para realizar construcciones funerarias dentro de cementerios municipales.',
                    'icon' => 'flower-outline',
                    'color' => 'primary',
                    'legal_base' =>
                        'Artículos 13, 46, 50, 56 y 57 del Reglamento de Panteones y Cementerios del Municipio',
                    'costs' => [
                    ...
                    <div class="card card-image card-image-banner wow fadeInUp">
                        ...
                        <div class="card-content">
                            <div class="d-flex align-items-center mb-3">
                                <div class="card-icon card-icon-static bg-{{ $tramite_actual['color'] }} text-white d-flex align-items-center justify-content-center me-3"
                ...
                <div class="col-md-12">
                    <div class="card wow fadeInUp">
                        <div class="card-body">
                            ...
                            @if (!empty($tramite_actual['variants']))
                                <!-- Este trámite tiene variantes con distintos requisitos -->
                                @php
                                    $variantColors = [
                                        'primary' => '#007bff',
                                        'info' => '#17a2b8',
                                    ...
                                    $variantActiveBg = $variantColors[$tramite_actual['color']] ?? '#007bff';
                                @endphp
                                <ul class="nav nav-pills variant-pills mb-4" id="variantsTab" role="tablist"
                                ...
                                <div class="tab-content" id="variantsTabContent">
                                    @foreach ($tramite_actual['variants'] as $vIndex => $variant)
                                        <div class="tab-pane fade {{ $vIndex === 0 ? 'show active' : '' }}"
                                            id="variant-pane-{{ $vIndex }}" role="tabpanel"
                                            aria-labelledby="variant-tab-{{ $vIndex }}">
                                            <h5 class="mb-4 text-{{ $tramite_actual['color'] }}">{{ $variant['title'] }}
                                            </h5>
                        ...
                        <div class="card-body">
                            <div class="row mb-4">
                                @foreach ($tramite_actual['costs'] as $index => $cost)
                                    <div
                                        class="col-md-{{ count($tramite_actual['costs']) == 1 ? '12' : (count($tramite_actual['costs']) == 2 ? '6' : '4') }} mb-3">
                            ...
                            <div class="alert alert-info mt-4">
                                ...
                                <p class="mb-0">Los costos están basados en la Ley de Ingresos vigente del Municipio de
                                    Valle de Santiago. Los precios pueden variar según actualizaciones periódicas. Para
                                    información actualizada, consulte en Tesorería Municipal.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-12 text-center">
                    <div class="card wow fadeInUp">
                        <div class="card-body py-5">
                            ...
                            <p class="mb-4">Una vez que tengas todos los documentos listos, puedes iniciar el proceso en
                                línea.</p>

                            <a href="{{ route('citizen.urban_dev.create') }}"
                                class="btn btn-{{ $tramite_actual['color'] }} btn-lg px-5 py-3">
                                <ion-icon name="documents-outline" class="me-2"></ion-icon>
                                Iniciar Trámite
                            </a>

                            <p class="mt-2"><small>Para los trámites debes Iniciar Sesión y tener una cuenta en nuestro
                                    sistema. Registrate <a href="{{ route('register') }}">aquí</a></small></p>
