                ...
                <div class="card wow fadeInUp">
                    <div class="card-body">
                        ...
                        @switch($type)
                            ...
                            @case('medio-ambiente')
                                <h5 class="mb-4">
                                    <ion-icon name="leaf-outline"></ion-icon> Dirección de Medio Ambiente — Mis Solicitudes
                                </h5>
                                <div class="row">
                                    @foreach (\App\Models\EnvironmentRequest::REQUEST_TYPES as $requestType => $requestLabel)
                                        <div class="col-md-4 mb-4">
                                            <div class="card h-100 border">
                                                <div class="card-body d-flex flex-column">
                                                    <div class="mb-3">
                                                        <span class="badge bg-success bg-opacity-10 text-success px-3 py-2 mb-2">
                                                            <ion-icon name="leaf-outline" class="me-1"></ion-icon> Medio Ambiente
                                                        </span>
                                                    </div>
                                                    <h5 class="card-title fw-semibold">Solicitudes de {{ Str::title(explode(' ', $requestLabel, 2)[1] ?? $requestLabel) }}</h5>
                                                    <p class="card-text text-muted flex-grow-1">
                                                        Gestiona y da seguimiento a tus solicitudes ciudadanas de {{ strtolower($requestLabel) }}.
                                                    </p>
                                                    <div class="d-flex gap-2 mt-auto">
                                                        <a href="{{ route('citizen.environment.create', $requestType) }}" class="btn btn-primary flex-grow-1">
                                                            <ion-icon name="add-outline"></ion-icon> Nueva Solicitud
                                                        </a>
                                                        <a href="{{ route('citizen.environment.requests', $requestType) }}" class="btn btn-outline-secondary flex-grow-1">
                                                            <ion-icon name="eye-outline"></ion-icon> Ver solicitudes
                                                        </a>
                                                    </div>
                                                </div>
