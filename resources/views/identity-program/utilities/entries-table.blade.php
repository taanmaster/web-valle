<div>
    ...
    <div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
        <h4 class="fw-bold text-uppercase mb-0">Programa de Identidad</h4>
        <div class="d-flex gap-2">
            <button wire:click="toggleFilters" class="btn btn-secondary fw-semibold px-4">Filtros</button>
            <a href="{{ route('identity_program.admin.create') }}" class="btn btn-primary fw-semibold px-4">Nueva
                Entrada</a>
        </div>
    </div>
    ...
    <div class="row g-4">
            ...
            <div class="col-md-6" wire:key="entry-{{ $entry->id }}">
                <div class="card border-0 shadow-sm h-100" style="border-radius: 12px; overflow: hidden;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('identity_program.admin.show', $entry->id) }}"
                                class="btn btn-sm btn-outline-secondary flex-fill">
                                <i class="bx bx-show"></i> Ver
                            </a>
                            <a href="{{ route('identity_program.admin.edit', $entry->id) }}"
                                class="btn btn-sm btn-outline-primary flex-fill">
                                <i class="bx bx-edit"></i> Editar
                            </a>
                            <button wire:click="delete({{ $entry->id }})"
                                wire:confirm="¿Eliminar esta entrada? Esta acción no se puede deshacer."
        ...
        @empty
            <div class="col-12 text-center py-5 text-muted">No hay entradas registradas.</div>
        @endforelse
    </div>
