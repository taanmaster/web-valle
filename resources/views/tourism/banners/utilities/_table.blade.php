            ...
            @foreach ($banners as $banner)
                <tr>
                    ...
                    <td class="d-flex gap-3">
                        <a href="{{ route('tourism.banners.show', $banner->id) }}" class="btn btn-primary"
                            data-toggle="tooltip" data-original-title="Ver Detalle">
                            Ver Detalle
                        </a>

                        <a href="{{ route('tourism.banners.edit', $banner->id) }}" class="btn btn-primary"
                            data-toggle="tooltip" data-original-title="Editar">
                            Editar
                        </a>

                        <form method="POST" action="{{ route('tourism.banners.status', $banner->id) }}">
                            {{ csrf_field() }}
                        ...
                        <form method="POST" action="{{ route('tourism.banners.destroy', $banner->id) }}">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
