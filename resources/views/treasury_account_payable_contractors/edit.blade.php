...
@slot('title') Editar Contratista @endslot
@endcomponent
        ...
        <div class="row">
            <div class="col-8">
                <div class="card card-body">
                    <form method="POST" action="{{ route('treasury_account_payable_contractors.update', $contractor->id) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <div class="row">
                            <!-- Información básica -->
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" id="rfc" name="rfc" value="{{ $contractor->rfc }}">
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" id="phone" name="phone" value="{{ $contractor->phone }}">
                            </div>

                            <!-- Información de la cuenta bancaria -->
                            <div class="col-md-6 mb-3">
                                <label for="account_name" class="form-label">Nombre de la Cuenta <span class="text-info tx-12">(Opcional)</span></label>
                                <input type="text" class="form-control" id="account_name" name="account_name" value="{{ $contractor->account_name }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="account_number" class="form-label">Número de Cuenta <span class="text-info tx-12">(Opcional)</span></label>
                                <input type="text" class="form-control" id="account_number" name="account_number" value="{{ $contractor->account_number }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="bank_name" class="form-label">Banco <span class="text-info tx-12">(Opcional)</span></label>
                                <input type="text" class="form-control" id="bank_name" name="bank_name" value="{{ $contractor->bank_name }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="dependency_name" class="form-label">Dependencia <span class="text-info tx-12">(Opcional)</span></label>
                                <input type="text" class="form-control" id="dependency_name" name="dependency_name" value="{{ $contractor->dependency_name }}">
                            </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="form-control" id="status" name="status" required>
                                    <option value="active" {{ $contractor->status === 'active' ? 'selected' : '' }}>Activo</option>
                                    <option value="inactive" {{ $contractor->status === 'inactive' ? 'selected' : '' }}>Inactivo</option>
                                </select>
                            </div>
                        </div>

                        <button type="submit" class="btn btn-dark btn-sm">Guardar cambios</button>
                        <a href="{{ route('treasury_account_payable_contractors.index') }}" class="btn btn-secondary btn-sm">Cancelar</a>
                    </form>
                </div>
            </div>
        </div>
